...
export class EdgeHandlers {
  ...
  /*
    ...
    Пофиксили это добавлением флага, мы делаем unbind если делаем unmount (или не делали mount вообще). 
    И при отписке во время удаления элемента мы в unbind больше не заходим.
  */
  bindEvents() {
    if (!this.isBinded) {
      this.isBinded = true;

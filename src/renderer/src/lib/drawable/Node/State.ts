...
export class State extends Shape {
  ...
  constructor(editorView: EditorView, id: string, parent?: Shape) {
    ...
    this.eventBox = new Events(this.editorView, this);
    this.updateEventBox();
    this.edgeHandlers = new EdgeHandlers(editorView.app, this);
  }

  get data() {
    return this.editorView.app.model.data.elements.states[this.id];
  }
  ...
  draw(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) {
    ...
    if (this.editorView.editorController.states.dragInfo?.parentId === this.id) {
  ...
  private drawBody(ctx: CanvasRenderingContext2D) {
    ...
    ctx.roundRect(x, y, width, height, [
      6 / this.editorView.app.model.data.scale,
      6 / this.editorView.app.model.data.scale,
      (this.children.isEmpty ? 6 : 0) / this.editorView.app.model.data.scale,
      (this.children.isEmpty ? 6 : 0) / this.editorView.app.model.data.scale,
    ]);
    ctx.fill();
  ...
  get computedTitleSizes() {
    return {
      height: this.titleHeight / this.editorView.app.model.data.scale,
      width: this.drawBounds.width,
      fontSize: 15 / this.editorView.app.model.data.scale,
      paddingX: 15 / this.editorView.app.model.data.scale,
      paddingY: 10 / this.editorView.app.model.data.scale,
    };
  }
  ...
  private drawSelection(ctx: CanvasRenderingContext2D) {
    ...
    ctx.beginPath();
    ctx.roundRect(x, y, width, height + childrenHeight, 6 / this.editorView.app.model.data.scale);
    ctx.stroke();
    ctx.closePath();
    ...
    ctx.moveTo(x + 100 / this.editorView.app.model.data.scale, y - 20 / this.editorView.app.model.data.scale);
    ctx.lineTo(x + 110 / this.editorView.app.model.data.scale, y - 2 / this.editorView.app.model.data.scale);
    ctx.lineTo(x + 120 / this.editorView.app.model.data.scale, y - 20 / this.editorView.app.model.data.scale);
    //Строит прямоугольник
    ctx.roundRect(
      x,
      y - 120 / this.editorView.app.model.data.scale,
      width,
      100 / this.editorView.app.model.data.scale,
    ...
    ctx.fillText(this.isState, x, y - 80 / this.editorView.app.model.data.scale);
    //Добавляет задний фон объекту канвы
    ctx.fill();
    ctx.closePath();
    */
  ...
  private drawPen(ctx: CanvasRenderingContext2D) {
    ...
    const size = 16 / this.editorView.app.model.data.scale;
    const p = 9 / this.editorView.app.model.data.scale;
